...
def main():
    ...
    url = 'https://docs.contrastsecurity.jp/ja/php-agent-release-notes-and-archive.html'
    res = req.urlopen(url)
    ...
    feed = Rss201rev2Feed(
        title='PHP Agent Release Note',
        link='https://contrastsecurity.dev/contrast-documentation-rss',
        description='PHP Agent Release Note',
        language='ja',
        author_name="Contrast Security Japan G.K.",
        feed_url='https://contrastsecurity.dev/contrast-documentation-rss/php_rlsnote.xml',
    ...
    for elem in elems:
        try:
            ...
            if not title.lower().startswith('php'):
                continue
    ...
    with open('/feeds/php_rlsnote.xml','w') as fp:
