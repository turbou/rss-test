...
def main():
    ...
    feed = Rss201rev2Feed(
        title='Node.js Agent Beta Release Note',
        link='https://contrastsecurity.dev/contrast-documentation-rss',
        description='Node.js Agent Beta Release Note',
        language='ja',
        author_name="Contrast Security Japan G.K.",
        feed_url='https://contrastsecurity.dev/contrast-documentation-rss/nodejs_beta_rlsnote.xml',
    ...
    for elem in elems:
        try:
            ...
            if not (title.lower().startswith('node') and re.search(r'\d$', id_str)):
                continue
    ...
    with open('/feeds/nodejs_beta_rlsnote.xml','w') as fp:
