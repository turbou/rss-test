...
def main():
    ...
    url = 'https://docs.contrastsecurity.jp/ja/ruby-agent-release-notes-and-archive.html'
    res = req.urlopen(url)
    ...
    feed = Rss201rev2Feed(
        title='Ruby Agent Release Note',
        link='https://contrastsecurity.dev/contrast-documentation-rss',
        description='Ruby Agent Release Note',
        language='ja',
        author_name="Contrast Security Japan G.K.",
        feed_url='https://contrastsecurity.dev/contrast-documentation-rss/ruby_rlsnote.xml',
    ...
    for elem in elems:
        try:
            ...
            if not title.lower().startswith('ruby'):
                continue
    ...
    with open('/feeds/ruby_rlsnote.xml','w') as fp:
