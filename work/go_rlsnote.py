...
def main():
    ...
    url = 'https://docs.contrastsecurity.jp/ja/go-agent-release-notes-and-archive.html'
    res = req.urlopen(url)
    ...
    feed = Rss201rev2Feed(
        title='Go Agent Release Note',
        link='https://contrastsecurity.dev/contrast-documentation-rss',
        description='Go Agent Release Note',
        language='ja',
        author_name="Contrast Security Japan G.K.",
        feed_url='https://contrastsecurity.dev/contrast-documentation-rss/go_rlsnote.xml',
    ...
    for elem in elems:
        try:
            ...
            if not title.lower().startswith('go'):
                continue
    ...
    with open('/feeds/go_rlsnote.xml','w') as fp:
