...
def main():
    ...
    url = 'https://docs.contrastsecurity.jp/ja/node-js-agent-release-notes-and-archive.html'
    res = req.urlopen(url)
    ...
    feed = Rss201rev2Feed(
        title='Node.js Agent Protect Release Note',
        link='https://contrastsecurity.dev/contrast-documentation-rss',
        description='Node.js Agent Protect Release Note',
        language='ja',
        author_name="Contrast Security Japan G.K.",
        feed_url='https://contrastsecurity.dev/contrast-documentation-rss/nodejs_protect_rlsnote.xml',
    ...
    for elem in elems:
        try:
            ...
            title = elem.select('h4.title')[0].text.strip()
            if not (title.lower().startswith('node') and re.search(r'protect$', id_str)):
                continue
    ...
    with open('/feeds/nodejs_protect_rlsnote.xml','w') as fp:
